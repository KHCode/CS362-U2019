...
int baronEffect(struct gameState *state, int discardEstate, int currentPlayer) {
	...
	if (discardEstate) {//player decided to discard an estate
		int cardIterator = 0;//Iterator for hand!
		int card_not_discarded = 1;//Flag for discard set!
		while (card_not_discarded) {
			int coins = state->coins;
			int currentCard = state->hand[currentPlayer][cardIterator];
			if (currentCard == estate) {//Found an estate card!
				coins += 4;//Add 4 coins to the amount of coins
				discardCard(currentCard, currentPlayer, state, 0);//trash the estate card
				card_not_discarded = 0;//Exit the loop
			}
			else if (cardIterator > state->handCount[currentPlayer]) {
				if (DEBUG) {
					printf("No estate cards in your hand, invalid choice\n");
					printf("Must gain an estate if there are any\n");
				}
				int gainEstate = gainCard(estate, state, 0, currentPlayer);
				if (gainEstate == -1) {
					printf("Sorry, no more estate cards!\n");
				}
				card_not_discarded = 0;//Exit the loop
			}
			else {
...
int minionEffect(struct gameState* state, int choice, int currentPlayer, int handPos) {
	state->numActions++;
...
int ambassadorEffect(struct gameState* state, int cardChoice, int returnToSupply, int currentPlayer, int handPos) {
	...
	for (int j = 0; j < returnToSupply; j++) {	//discard copies of cards returned to supply
		for (int i = 0; i < state->handCount[currentPlayer]; i++) {
...
int tributeEffect(struct gameState* state, int currentPlayer, int nextPlayer, int* tributeRevealedCards) {
	...
	for (int i = 0; i < 2; i++) {	//reveal the two cards from the next player's deck
		tributeRevealedCards[i] = state->deck[nextPlayer][state->deckCount[nextPlayer]];
...
int mineEffect(struct gameState* state, int currentPlayer, int trashCard, int upgradeCard, int handPos) {
	int toTrash = state->hand[currentPlayer][trashCard];  //store card we will trash

	if (state->hand[currentPlayer][trashCard] < copper || state->hand[currentPlayer][trashCard] > gold) {
		return -1;
	}

	if (upgradeCard > copper || upgradeCard > gold) {
		return -1;
	}

	if ((getCost(state->hand[currentPlayer][trashCard]) + 3) > getCost(upgradeCard)) {
		return -1;
	}

	gainCard(upgradeCard, state, 2, currentPlayer);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case minion:
		minionEffect(state, choice1, currentPlayer, handPos);
		return 0;
